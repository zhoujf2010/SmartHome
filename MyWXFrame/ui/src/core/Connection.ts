...
export function msgerror(code: any, message: string) {
  return {
    type: "result",
    success: false,
    error: {
...
export function msgunsubscribeEvents(subscription: number) {
  return {
...
export class Connection {
  ...
  constructor(socket: WebSocket, auth: Auth) {
    // connection options
  ...
  addEventListener(eventType: any, callback: ConnectionEventListener) {
  ...
  removeEventListener(eventType: any, callback: ConnectionEventListener) {
  ...
  fireEvent(eventType: any, eventData?: any) {
    ...
    switch (message.type) {
      case "event":
        if (info) {
          ...
          this.sendMessagePromise(msgunsubscribeEvents(message.id));
        }
        break;
  ...
  private async _handleClose(ev: CloseEvent) {
    ...
    this.commands.forEach((info) => {
      ...
      if (!("subscribe" in info)) {
        info.reject(msgerror(ERR_CONNECTION_LOST, "Connection lost"));
      }
    });
    ...
    const reconnect = (tries: number) => {
      setTimeout(async () => {
        ...
        try {
          const socket = await createSocket(this.auth);
          this.setSocket(socket);
        } catch (err) {
          if (this._queuedMessages) {
            const queuedMessages = this._queuedMessages;
...
export async function createConnection(auth: Auth) {
  const socket = await createSocket(auth);
  const conn = new Connection(socket, auth);
  return conn;
}

export function msgauth(accessToken: string) {
  return {
    type: "auth",
...
export function createSocket(auth: Auth): Promise<WebSocket> {
    ...
    try {
      ...
      const handleOpen = async (event: MessageEventInit) => {
        try {
          ...
          socket.send(JSON.stringify(msgauth(auth.accessToken)));
        } catch (err) {
...
const handleFetchPromise = async <T>(
