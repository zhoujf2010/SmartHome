...
import { dynamicElement } from "../dynamic-element-directive";
...
export class HaForm extends LitElement implements HaFormElement {
  ...
  @property() public computeError?: (schema: HaFormSchema, error:string) => string;
  ...
  constructor() {
    super();
    this.error = "";//new Record<string, string>();
  }
  ...
  private _computeError(error:string, schema: HaFormSchema ) {
    return this.computeError ? this.computeError(schema,error) : error;
  }

    // @ts-ignore: Unreachable code error
  private _getValue(obj:HaFormData, item) {
    if (obj) {
      // @ts-ignore: Unreachable code error
